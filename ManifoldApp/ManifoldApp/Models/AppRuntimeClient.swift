...
actor FixtureRuntimeClient: RuntimeClientProtocol {
    ...
    private static func makeDemoState() -> FixtureState {
        ...
        return FixtureState(
            ...
            activeGrantEmailIDs: ["demo-m365-github", "demo-m365-stripe", "demo-m365-boris", "demo-m365-build", "demo-m365-data", "demo-m365-incident", "demo-m365-pr143", "demo-m365-boris-pr143", "demo-m365-marc-oauth", "demo-m365-stripe-story", "demo-m365-build-pr143", "demo-m365-data-story", "demo-m365-ops-rollback", "demo-m365-codex-session"],
            pendingApprovals: [
            ...
            sharedEmailIDsByAgent: [
                .cowork: ["demo-icloud-roadmap", "demo-icloud-pricing", "demo-gmail-interview", "demo-icloud-support", "demo-icloud-launch", "demo-icloud-calendar", "demo-gmail-survey", "demo-m365-design", "demo-m365-template", "demo-icloud-donatella-roadmap", "demo-icloud-tom-pricing", "demo-icloud-mae-copy", "demo-icloud-support-vip", "demo-icloud-calendar-rehearsal", "demo-gmail-linen-feedback", "demo-gmail-retail-signal", "demo-m365-pixel-storyboard", "demo-m365-session-librarian", "demo-icloud-mario-approval", "demo-gmail-donatella-persona", "demo-icloud-claude-session"],
                .codex: ["demo-m365-github", "demo-m365-stripe", "demo-m365-boris", "demo-m365-build", "demo-m365-data", "demo-m365-incident", "demo-m365-pr143", "demo-m365-boris-pr143", "demo-m365-marc-oauth", "demo-m365-stripe-story", "demo-m365-build-pr143", "demo-m365-data-story", "demo-m365-ops-rollback", "demo-m365-codex-session"],
            ],
            sharedEmailAttachmentIDsByAgent: [:],
            fileVisibilityOverrides: [
                .cowork: [
    ...
    private static func demoEmails() -> [EmailMessageRecord] {
        [
            ...
            demoEmail("demo-icloud-donatella-roadmap", "demo-icloud", "Donatella Amodei <donatella@anthropologie.test>", "donatella@anthropologie.test", "anthropologie.test", "Roadmap sequel - Mario needs a cleaner arc", "2026-05-02T09:14:00Z", "Claude shared. Donatella wants the Q3 roadmap framed as a story about trust, not just feature velocity."),
            demoEmail("demo-icloud-tom-pricing", "demo-icloud", "Tom Brownie <tom@anthropologie.test>", "tom@anthropologie.test", "anthropologie.test", "Scarf margin table v2", "2026-05-02T09:32:00Z", "Claude shared. Tom asks whether the flat pricing decision still works once Mae adds the window display bundle."),
            demoEmail("demo-icloud-mae-copy", "demo-icloud", "Mae Ribbon <mae@anthropologie.test>", "mae@anthropologie.test", "anthropologie.test", "Launch copy needs one human sentence", "2026-05-02T09:48:00Z", "Claude shared. Mae wants the linen launch copy to mention Mario's approval path without sounding procedural."),
            demoEmail("demo-icloud-support-vip", "demo-icloud", "Support Queue <support@anthropologie.test>", "support@anthropologie.test", "anthropologie.test", "VIP escalation for Donatella", "2026-05-02T10:04:00Z", "Claude shared. Support asks for a privacy-safe reply that keeps Donatella's board context out of the macro."),
            demoEmail("demo-icloud-calendar-rehearsal", "demo-icloud", "Calendar <calendar@anthropologie.test>", "calendar@anthropologie.test", "anthropologie.test", "Demo rehearsal moved after coffee tasting", "2026-05-02T10:18:00Z", "Claude shared. Mario moved the rehearsal so Demi can join after the track walk."),
            demoEmail("demo-gmail-linen-feedback", "demo-gmail", "Research Panel <panel@survey.test>", "panel@survey.test", "survey.test", "Linen launch pulse ready", "2026-05-02T10:35:00Z", "Claude shared after redaction. Customer feedback says Mae's copy lands better when paired with Donatella's trust story."),
            demoEmail("demo-gmail-retail-signal", "demo-gmail", "Retail Futures <newsletter@retail.test>", "newsletter@retail.test", "retail.test", "Merchandising signal: story-led launches", "2026-05-02T10:48:00Z", "Claude can use this as market context for Mario's board demo."),
            demoEmail("demo-m365-pixel-storyboard", "demo-m365", "Pixel Desk <design@anthropologie.org>", "design@anthropologie.org", "anthropologie.org", "Mario access storyboard exported", "2026-05-02T11:05:00Z", "Claude shared. The storyboard shows which demo cards reveal Donatella, Mae, and Tom."),
            demoEmail("demo-m365-session-librarian", "demo-m365", "Session Librarian <sessions@anthropologie.org>", "sessions@anthropologie.org", "anthropologie.org", "Character summary prompt added", "2026-05-02T11:22:00Z", "Claude shared. The saved prompt asks Claude to explain what each character can and cannot access."),
            demoEmail("demo-m365-pr143", "demo-m365", "GitHub <support@github.test>", "support@github.test", "github.test", "[github] PR #143 story access narrative", "2026-05-02T12:01:00Z", "Shared with Codex. Marc Chenille added an accessNarrative helper and Boris requested tests."),
            demoEmail("demo-m365-boris-pr143", "demo-m365", "Boris Cherrybomb <boris@anthropologie.org>", "boris@anthropologie.org", "anthropologie.org", "Re: PR #143 auth edge case still bites", "2026-05-02T12:12:00Z", "Shared with Codex. Boris says the staged OAuth callback loses the null guard when Marc's helper runs first."),
            demoEmail("demo-m365-marc-oauth", "demo-m365", "Marc Chenille <marc@openai.test>", "marc@openai.test", "openai.test", "OAuth callback fixture for Codex", "2026-05-02T12:24:00Z", "Shared with Codex through explicit PR context. Marc included a safe fixture and asked Codex to avoid .env."),
            demoEmail("demo-m365-stripe-story", "demo-m365", "Stripe <accounts@stripe.test>", "accounts@stripe.test", "stripe.test", "Webhook replay for story bundle", "2026-05-02T12:38:00Z", "Shared with Codex. Stripe failures now include Mae's window display invoice and Tom's margin check."),
            demoEmail("demo-m365-build-pr143", "demo-m365", "Build Bot <ci@github.test>", "ci@github.test", "github.test", "PR #143 checks failing on accessNarrative.test.ts", "2026-05-02T12:51:00Z", "Shared with Codex. The failing test proves Codex should see code/story but not legal/story."),
            demoEmail("demo-m365-data-story", "demo-m365", "Data Warehouse <warehouse@anthropologie.org>", "warehouse@anthropologie.org", "anthropologie.org", "Character access graph exported", "2026-05-02T13:06:00Z", "Shared with Codex. The graph links Mario to roadmap, Boris to PR #143, and Mae to launch copy."),
            demoEmail("demo-m365-ops-rollback", "demo-m365", "Ops Desk <ops@anthropologie.org>", "ops@anthropologie.org", "anthropologie.org", "Rollback runbook for Boris deploy", "2026-05-02T13:20:00Z", "Shared with Codex for the deploy rollback runbook after the auth narrative patch."),
            demoEmail("demo-m365-demi-drill", "demo-m365", "Demi Hassabit <demi@deepmind.test>", "demi@deepmind.test", "deepmind.test", "Privacy drill notes after track walk", "2026-05-02T13:41:00Z", "Not shared. Demi mixes the privacy drill with Mario's kart telemetry, so it stays out of agent context."),
            demoEmail("demo-m365-mustafa-partnership", "demo-m365", "Mustafa Sailman <mustafa@microsoft.test>", "mustafa@microsoft.test", "microsoft.test", "Partnership follow-up for linen story", "2026-05-02T14:02:00Z", "Not shared. Forwarded to Tom for commercial review, but legal redlines remain hidden."),
            demoEmail("demo-m365-sam-working-group", "demo-m365", "Sam Altmore <sam@openai.test>", "sam@openai.test", "openai.test", "Working group NDA next steps", "2026-05-02T14:26:00Z", "Not shared. Legal and working-group notes stay outside the demo access grant."),
            demoEmail("demo-gmail-kart-alibi", "demo-gmail", "Vintage Kart Forum <vintage-kart@oldmotors.test>", "vintage-kart@oldmotors.test", "oldmotors.test", "Mario's Saturday alibi thread", "2026-05-02T14:45:00Z", "Not shared. Racing details remain Mario's private side story."),
            demoEmail("demo-gmail-bank-launch", "demo-gmail", "Bank Alerts <alerts@bank.test>", "alerts@bank.test", "bank.test", "Launch wire confirmation", "2026-05-02T15:02:00Z", "Blocked by privacy filter before sharing."),
            demoEmail("demo-icloud-vendor-window", "demo-icloud", "Fabric Samples <orders@fabric.test>", "orders@fabric.test", "fabric.test", "Mae window display receipt", "2026-05-02T15:18:00Z", "Not shared. Vendor receipt stays outside AI even though Mae's public copy is shared."),
            demoEmail("demo-m365-sales-followup", "demo-m365", "Pipeline Bot <pipeline@sales.test>", "pipeline@sales.test", "sales.test", "Mustafa retail collaboration rows", "2026-05-02T15:35:00Z", "Claude can summarize only redacted pipeline rows for the partnership story."),
            demoEmail("demo-m365-legal-redline", "demo-m365", "Legal Desk <legal@anthropologie.org>", "legal@anthropologie.org", "anthropologie.org", "Mustafa partnership redline", "2026-05-02T15:52:00Z", "Not shared. Legal redline remains blocked even when the sales story is visible."),
            demoEmail("demo-icloud-mario-approval", "demo-icloud", "Mario Amodei <mario@anthropologie.test>", "mario@anthropologie.test", "anthropologie.test", "Approval path for launch day", "2026-05-02T16:10:00Z", "Claude shared. Mario asks Claude to turn the approval path into a short briefing for Donatella."),
            demoEmail("demo-gmail-donatella-persona", "demo-gmail", "Research Panel <panel@survey.test>", "panel@survey.test", "survey.test", "Donatella persona synthesis ready", "2026-05-02T16:25:00Z", "Claude shared after redaction. The synthesis explains why Donatella keeps asking for fewer metrics and more consequences."),
            demoEmail("demo-m365-codex-session", "demo-m365", "Session Librarian <sessions@anthropologie.org>", "sessions@anthropologie.org", "anthropologie.org", "Codex Boris PR #143 session saved", "2026-05-02T16:42:00Z", "Shared with Codex. The session replay records the failing test, the rollback runbook, and the safe fixture."),
            demoEmail("demo-icloud-claude-session", "demo-icloud", "Session Librarian <sessions@anthropologie.org>", "sessions@anthropologie.org", "anthropologie.org", "Claude Donatella roadmap session saved", "2026-05-02T16:55:00Z", "Claude shared. The session replay follows Donatella from roadmap critique to launch-room copy."),
            demoEmail("demo-m365-privacy-regression", "demo-m365", "Demi Hassabit <demi@deepmind.test>", "demi@deepmind.test", "deepmind.test", "Story mode privacy regression", "2026-05-02T17:12:00Z", "Not shared by default. Demi's regression references sensitive drill notes and should stay hidden."),
        ]
    }
