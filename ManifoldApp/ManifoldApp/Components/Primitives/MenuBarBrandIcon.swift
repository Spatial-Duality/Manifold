...
//
// MenuBarBrandIcon — the Manifold mark with state badge, rendered for the
// macOS menu bar.
//
...
// non-system image. We render the BrandMark + badge through ImageRenderer
...
struct MenuBarBrandIcon: View {
    ...
    private var mark: some View {
        BrandMark(placement: .menubar, color: ManifoldPalette.text)
...
extension MenuBarBrandIcon {
    ...
    static func renderTemplateImage(state: MenuBarBadgeState) -> NSImage? {
        let view = StaticMenuBarBrandIcon(state: state)
...
private struct StaticMenuBarBrandIcon: View {
    let state: MenuBarBadgeState
...
#Preview("Menu bar icon — all states") {
    HStack(spacing: 16) {
        VStack { MenuBarBrandIcon(state: .clear);            Text(".clear").font(ManifoldType.tiny) }
        VStack { MenuBarBrandIcon(state: .approvalsPending); Text(".approvalsPending").font(ManifoldType.tiny) }
        VStack { MenuBarBrandIcon(state: .paused);           Text(".paused").font(ManifoldType.tiny) }
        VStack { MenuBarBrandIcon(state: .disconnected);     Text(".disconnected").font(ManifoldType.tiny) }
    }
    .padding(24)
