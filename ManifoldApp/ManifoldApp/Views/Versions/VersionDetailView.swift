...
// MARK: - Snapshot List Panel

private struct SnapshotListPanel: View {
    @Environment(ManifoldStore.self) var store
    let snapshots: [SnapshotRecord]
    @Binding var selectedSnapshot: SnapshotRecord?
    let restoredSnapshotID: Int?
...
private struct DiffPanel: View {
    @Environment(ManifoldStore.self) var store
    let selectedSnapshot: SnapshotRecord?
    let diffLines: [DiffLine]
    let loadingDiff: Bool
    @Binding var restoredSnapshotID: Int?
    let filePath: String
