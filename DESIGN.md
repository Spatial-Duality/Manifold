...
| 2026-04-28 | Mail inspector defaults closed, opens only on double-click | Single-click = select (Finder convention); double-click = open. The inspector also has a close button and a ⌥⌘0 toggle; the toolbar shows the same chord. |
| 2026-04-28 | TableColumn cells on macOS get explicit state + actions, not @Environment | SwiftUI Tables on macOS don't reliably propagate `@Observable` env values into TableColumn cells; reading the model from inside crashes layout (EXC_BREAKPOINT in `EnvironmentValues.subscript`). All Table cells take state via row props or explicit closures captured in the surrounding view body. |
